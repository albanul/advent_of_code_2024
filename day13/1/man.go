...
func CalculateSumOfTokens(games []Game) int {
	sum := 0
...
func TryToWin(game Game) (a, b int, err error) {
	...
	fb := float64(game.Prize.Y*b1.Dx-game.Prize.X*b1.Dy) / float64(b1.Dx*b2.Dy-b2.Dx*b1.Dy)
	b = int(fb)
	...
	fa := float64(game.Prize.X-b*b2.Dx) / float64(b1.Dx)
	a = int(fa)
	if fa != float64(a) {
		return a, b, errors.New("can't win")
	}
